...
func NewMcpToolProvider(name string, server shared.McpServerConfig) *McpClient {
	return &McpClient{
		...
		client: mcp.NewClient(&mcp.Implementation{
			Name:    "agent-web-base-mcp-client",
			Title:   "AgentWebBase",
			Version: "v1.0.0",
		}, nil),
...
func (t *McpTool) ToolName() tool.AgentTool {
	return tool.AgentTool(fmt.Sprintf("agent_web_base_mcp__%s__%s", t.client.Name(), t.toolName))
}
